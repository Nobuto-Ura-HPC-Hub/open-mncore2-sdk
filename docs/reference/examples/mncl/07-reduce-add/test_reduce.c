/* test_reduce.c -- reduce_add sample (4096 PE → 4 partial sums)
 *
 ...
 *
 * 動作モード:
 *   --verify あり : 入力を C で reduce_add し、 device 結果と verify。 結果を stdout に表示。 exit 0=PASS / 1=FAIL
...
#define OUTPUT_COUNT   4                            /* 4 partial sums */
...
static int run_verify(const double *input, const double *parts) {
    /* C 側: 全要素 add */
    double c_total = 0.0;
    for (int i = 0; i < ELEM_COUNT; i++) c_total += input[i];

    /* device 側: 4 partial sums を合計 */
    double dev_total = 0.0;
    for (int i = 0; i < OUTPUT_COUNT; i++) dev_total += parts[i];

    /* C 側: group ごと（interleaved mapping: PE i → group (i % OUTPUT_COUNT)）
     * device の partial sums は PE position 別 = i % 4 で分割される
     * (実機/emu:lib 計測で確認: parts[g] = sum of input[g, g+4, g+8, ...]) */
    double c_groups[OUTPUT_COUNT];
    int per_group_count = 0;
    for (int g = 0; g < OUTPUT_COUNT; g++) {
        c_groups[g] = 0.0;
        per_group_count = 0;
        for (int i = g; i < ELEM_COUNT; i += OUTPUT_COUNT) {
            c_groups[g] += input[i];
            per_group_count++;
        }
    }

    printf("=== reduce_add verify ===\n");
    printf("device parts (per group as returned by HW):\n");
    for (int g = 0; g < OUTPUT_COUNT; g++) {
        printf("  parts[%d] = %.15g\n", g, parts[g]);
    }
    printf("C per-group sum (interleaved, i %% %d == g, %d elems each):\n",
           OUTPUT_COUNT, per_group_count);
    for (int g = 0; g < OUTPUT_COUNT; g++) {
    ...
    double denom = fabs(c_total);
    if (denom == 0.0) denom = 1.0;
    double rel = fabs(c_total - dev_total) / denom;
    if (rel > 1e-10) {
        printf("\nFAIL (rel_err=%.3e)\n", rel);
        return 1;
    }
...
int main(int argc, char **argv) {
    ...
    if (verify) {
        printf("07-reduce-add backend: %s\n", mnc2_get_backend_name(dev));
