/* test_vecadd_2d.c — 2D vecadd E2E 検証 (emu:lib)
 *
 * c[i][j] = a[i][j] + b[i][j] を 64x64 PE で実行し、結果を検証する。
...
int main(void)
{
    printf("=== vecadd 2D: c[i][j] = a[i][j] + b[i][j] (%dx%d PE) ===\n\n",
           NI, NJ);
    ...
    printf("[exec] double-vecadd kernel (2D: %dx%d)\n", NI, NJ);
...

    {
        ...
        if (errors > 0) {
            ...
            printf("  PASS: c[i][j] == a[i][j] + b[i][j] for all %d elements\n",
                   ELEM_COUNT);
            rc = 0;
        }
    }
