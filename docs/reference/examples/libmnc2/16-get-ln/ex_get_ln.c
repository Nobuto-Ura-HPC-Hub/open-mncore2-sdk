/* ex_get_ln.c — get_ln collect roundtrip テスト
 *
 * PDM にデータを send し、put_ln で LM1 に distribute、
 * get_ln で collect して PDM に書き戻し、recv で検証。
 *
 */
...
int main(void)
{
    printf("[test] put+get LM1 roundtrip\n");
    ...
    k = mnc2_load_kernel(dev, "_build/get_ln.idma.dat");
    if (k == NULL) {
