/* ex_t6_maskr_pe3.c — T6: MASKR PE3 (ipassa + msl) テスト
 *
 * subpe[3] は ipassa (元値を保持)、他は msl 結果。
 * subpe 3:   expected = input[i]     (ipassa)
...
int main(void)
{
    printf("[test] T6: MASKR PE3 (ipassa on subpe[3], msl on others)\n");
    ...
    mnc2_kernel_t k = mnc2_load_kernel(dev, "_build/t6_maskr_pe3.idma.dat");
    if (k == NULL) {
