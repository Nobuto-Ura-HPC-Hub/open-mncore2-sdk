/* ex_get_lm.c — get_lm collect roundtrip テスト
 *
 * PDM にデータを send し、put_lm で LM0 に distribute、
 * get_lm で collect して PDM に書き戻し、recv で検証。
 *
 */
...
int main(void)
{
    printf("[test] put+get LM0 roundtrip\n");
    ...
    k = mnc2_load_kernel(dev, "_build/get_lm.idma.dat");
    if (k == NULL) {
