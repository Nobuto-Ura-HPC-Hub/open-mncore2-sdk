/* ex_put_grf0.c — put_grf distribute テスト
 *
 * PDM にデータを send し、put_grf カーネルで GRF0 に distribute。
 * mnc2_debug_read で GRF0 の値を読み出して非ゼロを確認。
 *
 */
...
int main(void)
{
    printf("[test] put → debug_read GRF0\n");
    ...
    /* device backend では mnc2_debug_read が動かない。put_grf 実行までで
       終わり、GRF0 の検証は build.ninja の peek pipeline (peek-check-device)
       で外部から行う。 */
    if (strcmp(mnc2_get_backend_name(dev), "device") == 0) {
        printf("[device] put_grf executed; debug_read skipped "
    ...
    rc = mnc2_debug_read(dev, MNC2_MEM_GRF0, &loc, 0, 4, out);
    if (rc != 0) {
