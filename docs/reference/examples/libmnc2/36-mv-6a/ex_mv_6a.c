/* ex_mv_6a.c — MV パターン 6A: DRAM@G0~3 → PDM@G0~3 個別並列 (3.5.8.3)
 *
 * 4 グループ各自の DRAM データを自グループの PDM に戻す。
...
int main(void)
{
    printf("[ex] MV 6A: DRAM@G0~3→PDM@G0~3 個別並列 (3.5.8.3) roundtrip\n");
    ...
    mnc2_kernel_t k = mnc2_load_kernel(dev, "_build/mv-p6a.idma.dat");
