/* ex_mv_2c.c — MV パターン 2C: PDM→L2BM 個別並列 (3.5.8.9)
 *
 * PDM@G[0..3][0..63] を L2BM@G.0[0..63] に個別並列転送後、
 * L2BM → PDM@G0~3 に戻して roundtrip を確認する。
 *
 */
#include <stdio.h>
#include <string.h>
...
int main(void)
{
    printf("[ex] MV 2C: PDM→L2BM 個別並列 (3.5.8.9) roundtrip\n");
    ...
    mnc2_kernel_t k = mnc2_load_kernel(dev, "_build/mv-p2c.idma.dat");
