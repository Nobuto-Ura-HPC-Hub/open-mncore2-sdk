/* ex_t2_msl.c — T2: MSL (Move Shift Left) テスト
 *
 * msl: PE[i] ← PE[i-1]、MAB 内 wrap。
 * subpe 0:     expected = input[i+3] (MAB 末尾から wrap)
 * subpe 1,2,3: expected = input[i-1]
 *
 */
...
int main(void)
{
    printf("[test] T2: MSL (PE[i] <- PE[i-1], MAB wrap)\n");
    ...
    mnc2_kernel_t k = mnc2_load_kernel(dev, "_build/t2_msl.idma.dat");
    if (k == NULL) {
