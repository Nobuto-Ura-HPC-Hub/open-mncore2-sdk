/* ex_mv_5a.c — MV パターン 5A: DRAM@G0~3 → PDM@G0 gather (4x individual mvp, 3.5.8.3)
 *
 * 4 グループの DRAM データを G0 の PDM に gather する。
...
int main(void)
{
    printf("[ex] MV 5A: DRAM@G0~3→PDM@G0 gather (4x mvp, 3.5.8.3)\n");
    ...
    mnc2_kernel_t k = mnc2_load_kernel(dev, "_build/mv-p5a.idma.dat");
