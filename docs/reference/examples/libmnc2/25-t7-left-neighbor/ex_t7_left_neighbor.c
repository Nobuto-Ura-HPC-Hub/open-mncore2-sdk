/* ex_t7_left_neighbor.c — T7: Left Neighbor (msr + l1bmd+1 + maskr) テスト
 *
 * msr + l1bmd+1 + maskr の組み合わせ。
 * subpe 0:   l1bmd+1 = MAB[N-1].PE[0]、L1B 内 wrap (16 MAB/L1B)
 * subpe 1,2: expected = input[i+1]   (msr)
 * subpe 3:   expected = input[i-3]   (msr wrap)
 *
 */
...
int main(void)
{
    printf("[test] T7: Left Neighbor (msr + l1bmd+1 + maskr)\n");
    ...
    mnc2_kernel_t k = mnc2_load_kernel(dev, "_build/t7_left_neighbor.idma.dat");
    if (k == NULL) {
    ...
    for (int i = 0; i < ELEM_COUNT; i++) {
        ...
        uint64_t expected;
        if (subpe == 0) {
            /* l1bmd+1: MAB[N-1].PE[0] with L1B-local wrap (16 MABs per L1B) */
            int l1b_base = (i / 64) * 64;
            int mab_in_l1b = (i / 4) % 16;
            int prev_mab = (mab_in_l1b - 1 + 16) % 16;
            expected = input[l1b_base + prev_mab * 4];
        } else if (subpe == 3) {
            /* msr wrap: PE[3] ← PE[0] of same MAB */
            expected = input[i - 3];
        } else {
            /* msr: PE[i] ← PE[i+1] */
            expected = input[i + 1];
