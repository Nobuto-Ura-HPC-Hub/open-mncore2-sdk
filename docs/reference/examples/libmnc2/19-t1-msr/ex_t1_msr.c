/* ex_t1_msr.c — T1: MSR (Move Shift Right) テスト
 *
 * msr: PE[i] ← PE[i+1]、MAB 内 wrap。
 * subpe 0,1,2: expected = input[i+1]
 * subpe 3:     expected = input[i-3] (MAB 先頭に wrap)
 *
 */
...
int main(void)
{
    printf("[test] T1: MSR (PE[i] <- PE[i+1], MAB wrap)\n");
    ...
    mnc2_kernel_t k = mnc2_load_kernel(dev, "_build/t1_msr.idma.dat");
    if (k == NULL) {
