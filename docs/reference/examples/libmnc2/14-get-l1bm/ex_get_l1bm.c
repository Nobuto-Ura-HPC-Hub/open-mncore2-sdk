/* ex_get_l1bm.c — get_l1bm collect roundtrip テスト
 *
 * PDM にデータを send し、put_l1bm で L1BM に distribute、
 * get_l1bm で collect して PDM に書き戻し、recv で検証。
 *
 */
...
int main(void)
{
    printf("[test] put+get L1BM roundtrip\n");
    ...
    k = mnc2_load_kernel(dev, "_build/get_l1bm.idma.dat");
    if (k == NULL) {
