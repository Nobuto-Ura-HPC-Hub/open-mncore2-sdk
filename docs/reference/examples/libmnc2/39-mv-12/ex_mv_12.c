/* ex_mv_12.c — MV パターン 12: DRAM→L2BM グループ間分配放送 (mvb4, 3.5.8.16)
 *
 * PDM@G0[0..63] を mvd distribute で DRAM@G[0..15] に分配後、
 * mvb4 で全グループ L2BM に分配放送し、
...
int main(void)
{
    printf("[ex] MV 12: DRAM→L2BM グループ間分配放送 (mvb4, 3.5.8.16) roundtrip\n");
    ...
    /* --- poison-fill: broadcast (mvb4) の wait 不足を単独実行で捕まえるガード ---
    ...
    mnc2_kernel_t k = mnc2_load_kernel(dev, "_build/mv-p12.idma.dat");
