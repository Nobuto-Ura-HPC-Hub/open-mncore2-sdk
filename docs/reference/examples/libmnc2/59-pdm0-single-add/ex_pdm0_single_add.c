/* ex_pdm0_single_add.c — 59: PDM@0 ↔ L2BM ↔ PE round trip (mvp 単独個別 8 発)
 *
 * 56/57/58 と並ぶベンチ系列の 4 例目。外部メモリ = PDM@0 のみ。
 * add1 1 iteration の PDM@0 ↔ L2BM hop は mvp 単独個別 を 8 + 8 命令で行う
 * (= 58 の mvd 1 + 1 = 2 命令との直接比較対象)。
 *
 * データフロー:
 *   1. host → PDM@0 (send): u64 配列 [i << 16 for i in 0..4095]
 *   2. Kernel A (distribute): no-op (host send tag i02 の wait のみ)
 *   3. Kernel B (add1):
 *        Phase 1: PDM@0 → 8 L2BMs (8 mvp 単独個別)
 *        Phase 2-6: L2BM → L1BM → LM → PE +1 → LM → L1BM → L2BM
 *        Phase 7: 8 L2BMs → PDM@0 (8 mvp 単独個別)
 *      N 回繰り返し
 *   4. Kernel C (collect):
 *        PE compute (flat PE ID = peid|(l1bid<<6)|(l2bid<<9) を $r0 に merge)
 *        → LM → L1BM → L2BM → 8 mvp 単独個別 L2BM-PDM
...
#define SEND_TAG    2
#define RECV_TAG    6
#define OUTPUT_FILE "_build/pdm0_single_add.bin"
...
int main(int argc, char** argv)
{
    ...
    printf("[test] PDM0-single repeat-add bijection (N=%d): %d elements, "
    ...
    if (strcmp(backend, "emu:process") == 0) {
        printf("SKIP: emu:process does not preserve PDM@0 state "
    ...
    /* Kernel A: distribute (no-op、host send tag の wait のみ) */
    ...
    /* Kernel B: PDM@0 ↔ L2BM ↔ PE +1, N 回繰り返し */
    ...
    /* Kernel C: PE 状態 ($lr0) を信頼して ID merge → L2BM → 8 mvp L2BM-PDM */
    ...
    /* 検証: bijection チェック */
    int n_mismatch = 0, i_bad = 0, peid_bad = 0, other_bits = 0;
    int dup_i = 0, dup_peid = 0;
