/* ex_t4_l1bmd_minus1.c — T4: L1BMD -1 方向観察テスト
 *
 * L1BMD -1 の方向を観察する。MAB[0-2] の PE[0-3] を出力して判定。
 * - output[4] == input[8] → MAB[N+1] から読み出し
 * 常に PASS（観察のみ）。
 *
 */
...
int main(void)
{
    printf("[test] T4: L1BMD -1 direction observation\n");
    ...
    mnc2_kernel_t k = mnc2_load_kernel(dev, "_build/t4_l1bmd_minus1.idma.dat");
    if (k == NULL) {
    ...
    } else if (output[4] == input[0]) {
        printf("  Direction: l1bmd-1 reads from MAB[N-1]\n");
    } else {
        printf("  Direction: undetermined (output[4]=%" PRIu64 ")\n",
               output[4]);
    }
