/* ex_put_lm.c — put_lm distribute テスト
 *
 * PDM にデータを send し、put_lm カーネルで LM0 に distribute。
 * mnc2_debug_read で LM0 の値を読み出して非ゼロを確認。
 *
 */
...
int main(void)
{
    printf("[test] put → debug_read LM0\n");
    ...
    mnc2_kernel_t k = mnc2_load_kernel(dev, "_build/put_lm.idma.dat");
    if (k == NULL) {
    ...
    /* device backend では mnc2_debug_read が動かない。put_lm 実行までで終わり、
       LM0 の検証は build.ninja の peek pipeline (peek-check-device) で外部から
       行う。 */
    if (strcmp(mnc2_get_backend_name(dev), "device") == 0) {
        printf("[device] put_lm executed; debug_read skipped "
    ...
    rc = mnc2_debug_read(dev, MNC2_MEM_LM0, &loc, 0, 4, out);
    if (rc != 0) {
