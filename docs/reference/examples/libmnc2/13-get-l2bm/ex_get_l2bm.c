/* ex_get_l2bm.c — get_l2bm collect roundtrip テスト
 *
 * PDM にデータを send し、put_l2bm で L2BM に distribute、
 * get_l2bm で collect して PDM に書き戻し、recv で検証。
 *
 */
...
int main(void)
{
    printf("[test] put+get L2BM roundtrip\n");
    ...
    k = mnc2_load_kernel(dev, "_build/get_l2bm.idma.dat");
    if (k == NULL) {
