/* ex_put_grf1.c — put_grf1 distribute テスト
 *
 * PDM にデータを send し、put_grf1 カーネルで GRF1 に distribute。
 * mnc2_debug_read で GRF1 の値を読み出して非ゼロを確認。
 *
 */
...
int main(void)
{
    printf("[test] put → debug_read GRF1\n");
    ...
    mnc2_kernel_t k = mnc2_load_kernel(dev, "_build/put_grf1.idma.dat");
    if (k == NULL) {
    ...
    /* device backend では mnc2_debug_read が動かない。put_grf1 実行までで
       終わり、GRF1 の検証は build.ninja の peek pipeline (peek-check-device)
       で外部から行う。 */
    if (strcmp(mnc2_get_backend_name(dev), "device") == 0) {
        printf("[device] put_grf1 executed; debug_read skipped "
    ...
    rc = mnc2_debug_read(dev, MNC2_MEM_GRF1, &loc, 0, 4, out);
    if (rc != 0) {
