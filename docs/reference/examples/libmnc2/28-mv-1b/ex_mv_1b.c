/* ex_mv_1b.c — MV パターン 1B: PDM→PDM scatter (mvb broadcast + mvp L2BM→PDM)
 *
 * mvb (3.5.8.18) で PDM@G0 → 全グループ L2BM に放送後、
 * mvp (3.5.8.10) で L2BM → PDM@G[0..3] に書き戻す。
 * 全 4 ブロックが送信データと一致することを確認する。
 *
 */
#include <stdio.h>
#include <string.h>
...
int main(void)
{
    printf("[ex] MV 1B: PDM→PDM scatter (mvb+mvp, 3.5.8.18+3.5.8.10)\n");
    ...
    for (int i = 0; i < BLOCK_ELEMS; i++)
        sb[i] = BLOCK_BASE[0] + (double)(i + 1);

    int rc = mnc2_send(dev, sbuf, 0, BLOCK_BYTES, 2);
    if (rc) { printf("FAIL: send rc=%d\n", rc); goto fail; }

    mnc2_kernel_t k = mnc2_load_kernel(dev, "_build/mv-p1b.idma.dat");
    ...
    for (int b = 0; b < 4; b++) {
        for (int i = 0; i < BLOCK_ELEMS; i++) {
            double exp = BLOCK_BASE[0] + (double)(i + 1);
