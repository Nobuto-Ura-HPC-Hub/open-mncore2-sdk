/* ex_put_ln.c — put_ln distribute テスト
 *
 * PDM にデータを send し、put_ln カーネルで LM1 に distribute。
 * mnc2_debug_read で LM1 の値を読み出して非ゼロを確認。
 *
 */
...
int main(void)
{
    printf("[test] put → debug_read LM1\n");
    ...
    mnc2_kernel_t k = mnc2_load_kernel(dev, "_build/put_ln.idma.dat");
    if (k == NULL) {
    ...
    /* device backend では mnc2_debug_read が動かない。put_ln 実行までで終わり、
       LM1 の検証は build.ninja の peek pipeline (peek-check-device) で外部から
       行う。 */
    if (strcmp(mnc2_get_backend_name(dev), "device") == 0) {
        printf("[device] put_ln executed; debug_read skipped "
    ...
    rc = mnc2_debug_read(dev, MNC2_MEM_LM1, &loc, 0, 4, out);
    if (rc != 0) {
