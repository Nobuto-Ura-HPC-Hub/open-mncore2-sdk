/* ex_t5_maskr_pe0.c — T5: MASKR PE0 (ipassa + msr) テスト
 *
 * subpe[0] は ipassa (元値を保持)、他は msr 結果。
 * subpe 0:   expected = input[i]     (ipassa)
...
int main(void)
{
    printf("[test] T5: MASKR PE0 (ipassa on subpe[0], msr on others)\n");
    ...
    mnc2_kernel_t k = mnc2_load_kernel(dev, "_build/t5_maskr_pe0.idma.dat");
    if (k == NULL) {
