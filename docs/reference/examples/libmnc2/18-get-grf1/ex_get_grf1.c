/* ex_get_grf1.c — get_grf1 collect roundtrip テスト
 *
 * PDM にデータを send し、put_grf1 で GRF1 に distribute、
 * get_grf1 で collect して PDM に書き戻し、recv で検証。
 *
 */
...
int main(void)
{
    printf("[test] put+get GRF1 roundtrip\n");
    ...
    k = mnc2_load_kernel(dev, "_build/get_grf1.idma.dat");
    if (k == NULL) {
